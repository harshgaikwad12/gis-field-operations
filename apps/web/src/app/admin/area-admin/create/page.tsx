...
function CreateAreaAdminForm() {
  ...
  useEffect(() => {
    ...
    async function loadAreas(authToken: string) {
      try {
        setAreasError("");
        const fetchedAreas = await getAreas(authToken);
        if (!cancelled) {
          setAreas(fetchedAreas);
          if (fetchedAreas.length > 0) {
            setSelectedAreaId(String(fetchedAreas[0].id));
          }
        }
      } catch (err) {
        if (!cancelled) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const numericAreaId = Number(selectedAreaId);
    ...
    try {
      const created = await createAreaAdmin(accessToken, {
      ...
      setSuccess(
        `Area Admin "${created.officer_name}" created successfully.`,
      );
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Unable to create area administrator.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <AppLayout
      ...
      subtitle="Provision new administrator credentials and assign to an area"
    >
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-slate-900 sm:text-xl">
            Area Administrator Details
          </h2>
          <p className="mt-1 text-sm text-slate-500">
            Create an administrator and assign the administrator to an area.
          </p>
        </div>
        ...
        {areasError && (
          ...
          >
            <p className="text-sm font-medium text-amber-700">{areasError}</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                placeholder="areaadmin@example.com"
                className="min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-slate-900 focus:ring-2 focus:ring-slate-900/10"
              />
            </div>
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs text-slate-400">
                Select the Area Name and Area Code to assign.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-1 text-xs text-slate-500">
              This form creates an Area Administrator assigned to the selected area.
            </p>
          </div>
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <button
              type="submit"
              disabled={loading || areasLoading || areas.length === 0}
            ...
            >
              {loading ? "Creating..." : "Create Area Admin"}
            </button>
          </div>
        </form>
      </section>
...
export default function CreateAreaAdminPage() {
  return (
